...
class _CollectionAndDeliveryPageState extends State<CollectionAndDeliveryPage> {
  ...
  void initState() { 
    ...
    widget.type == 0? title = '发货': title = '收货';
  }
    ...
    return Container(
      child: Column(
        children: <Widget>[
          ...
          textItem('$title重量',weightController, '请输入$title重量', false, true, 'number',(res){
            weightController.text = res.trim();
          }, (){}, '吨'),
          textItem('$title体积',volumeController, '请输入$title体积', false, true, 'number',(res){
            volumeController.text = res.trim();
          }, (){}, '方'),
          textItem('$title数量',numberController, '请输入$title数量', false, true, 'number',(res){
        ...
        children: <Widget>[
          ...
          :Stack(
            children: <Widget>[
              ...
              Positioned(
                child: InkWell(
                  child: Image.asset('assets/images/delete.png',width: ScreenUtil().setWidth(60),height: ScreenUtil().setHeight(60),),
                  onTap: (){
                  _closeImage(index);
                  },
                ),
  ...
  _collectionGoods(String loadingAttachment) {//装货
    ...
    postAjaxStr('$serviceUrl/waybill/waybill/${widget.code}/execute/loading', params, context).then((res){
      setState(() {
       _loading = false; 
      });
      if(res['code'] == 200) {
  ...
  _deliveryGoods(String loadingAttachment) {//卸货
    ...
    postAjaxStr('$serviceUrl/waybill/waybill/${widget.code}/execute/unloading', params, context).then((res){
      setState(() {
       _loading = false; 
      });
      if(res['code'] == 200) {
